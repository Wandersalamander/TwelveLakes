...
;
    ...
    public function makeRequest(lakeName as String, position as [Double,Double] or Array<Double>) as Void {
        ...
        var myRequest = alplakesApiString3DLake(lakeName, today, today, position);
      ...
      if (temperature != null){
        ...
        for (var i = 0; i < 6; i++) {
            ...
            if (colorTmp == colorHighlight){
              //dc.fillRectangle(i*barWidth, dc.getHeight()/2.0-heightHighlight/2.0, barWidth+delta, heightHighlight);
            }else{
        ...
        for (var i = 0; i < 6; i++) {
            ...
            dc.setColor(colorTmp, Graphics.COLOR_TRANSPARENT);
            if (colorTmp == colorHighlight){
              dc.fillRectangle(i*barWidth+(delta), dc.getHeight()/2.0-heightHighlight/2.0, barWidth-(2*delta), heightHighlight);
            }else{
              //dc.fillRectangle(i*barWidth, dc.getHeight()/2.0-height/2.0, barWidth+delta, height);
    ...
    public function onReceive3D(responseCode as Number, data as Dictionary or String or Null) as Void {
