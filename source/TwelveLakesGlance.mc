...
class WidgetGlanceView extends Ui.GlanceView {
  ...
  function onUpdate(dc) {
    ...
    if (temperature != null) {
      dc.drawText(
        ...
        Graphics.FONT_GLANCE,
        Lang.format("$1$ $2$°C", [
          readableLakeName(favouriteLake),
  ...
  public function makeRequest(forceUpdate as Boolean) as Void {
    ...
    if (lastUpdate != null) {
      ...
      readyForUpdate = Time.now().greaterThan(
        lastUpdate.add(skipUpdateBelowSeconds)
      );
    }
    if (forceUpdate) {
      readyForUpdate = true;
  ...
  function drawColorBar(dc as Gfx.Dc) as Void {
    ...
    if (temperature != null) {
      ...
      dc.fillRectangle(
        getColorPos(temperature) * dc.getWidth() - delta,
        center - heightMarker / 2.0,
        widthMarker + 2 * delta,
        heightMarker
      );
      dc.setColor(Graphics.COLOR_WHITE, Graphics.COLOR_TRANSPARENT);
      dc.fillRectangle(
        getColorPos(temperature) * dc.getWidth(),
        center - heightMarker / 2,
        widthMarker,
        heightMarker
      );
    }
